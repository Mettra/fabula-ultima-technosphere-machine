 ...
 */
export async function getPixelArtColors(
    imageUrl: string
): Promise<ImageColors> {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            if (!ctx) {
                reject(
                    new Error("Could not get 2D rendering context for canvas.")
                );
                return;
            }
            ...
            const calculateNormalizedHsvDistanceSq = (
                ...
                return (
                    dh_norm * dh_norm + ds_norm * ds_norm + dv_norm * dv_norm
                );
            };
            ...
            for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
                ...
                const nextPointsState = currentPointsState.map((point) => {
                    ...
                    for (const otherPoint of currentPointsState) {
                        const distSq = calculateNormalizedHsvDistanceSq(
                            point.currentHsv,
                            otherPoint.currentHsv
                    ...
                    totalMovementSq += calculateNormalizedHsvDistanceSq(
                        point.currentHsv,
                        shiftedHsv
                    ); // USE NORMALIZED DISTANCE
                ...
                currentPointsState = nextPointsState.map((p) => ({
                    ...p,
                    currentHsv: { ...p.newHsv },
                }));
                ...
                if (
                    currentPointsState.length > 0 &&
                    totalMovementSq / currentPointsState.length <
                        CONVERGENCE_THRESHOLD_SQ &&
                    iter > 0
                ) {
                    break; // Converged
                }
            ...
            const MERGE_THRESHOLD_SQ =
                CLUSTER_MERGE_THRESHOLD * CLUSTER_MERGE_THRESHOLD; // Now compares against normalized distance
            ...
            for (let i = 0; i < modes.length; i++) {
                ...
                let clusterSumH =
                    currentMode.convergedHsv.h * currentMode.originalCount;
                let clusterSumS =
                    currentMode.convergedHsv.s * currentMode.originalCount;
                let clusterSumV =
                    currentMode.convergedHsv.v * currentMode.originalCount;
                ...
                for (let j = i + 1; j < modes.length; j++) {
                    ...
                    if (
                        calculateNormalizedHsvDistanceSq(
                            currentMode.convergedHsv,
                            otherMode.convergedHsv
                        ) < MERGE_THRESHOLD_SQ
                    ) {
                        // USE NORMALIZED DISTANCE
                        assignedToCluster[j] = true;
                        clusterSumH +=
                            otherMode.convergedHsv.h * otherMode.originalCount;
                        clusterSumS +=
                            otherMode.convergedHsv.s * otherMode.originalCount;
                        clusterSumV +=
                            otherMode.convergedHsv.v * otherMode.originalCount;
            ...
            clusters.sort(
                (a, b) => b.totalOriginalCount - a.totalOriginalCount
            );
            ...
            if (clusters.length === 0) {
                ...
                if (initialPoints.length > 0) {
                    const mostFrequentInitial = [...initialPoints].sort(
                        (a, b) => b.count - a.count
                    )[0];
                    ...
                    reject(
                        new Error(
                            "Clustering resulted in no clusters and no fallback available."
                        )
                    );
                    return;
                }
                ...
                if (clusters.length > 1) {
                    ...
                    for (let i = 1; i < clusters.length; i++) {
                        ...
                        const distSqNorm = calculateNormalizedHsvDistanceSq(
                            primaryHsv,
                            candidateHsv
                        );
                        const hueDiffAbs = Math.min(
                            Math.abs(primaryHsv.h - candidateHsv.h),
                            360 - Math.abs(primaryHsv.h - candidateHsv.h)
                        );

                        if (
                            hueDiffAbs >= MIN_ABS_HUE_DIFF_SECONDARY ||
                            distSqNorm >= MIN_NORMALIZED_DIST_SQ_SECONDARY
                        ) {
            ...
            const finalPrimaryRgb = hsvToRgb(
                primaryHsv.h,
                primaryHsv.s,
                primaryHsv.v
            );
            const finalSecondaryRgb = hsvToRgb(
                secondaryHsv.h,
                secondaryHsv.s,
                secondaryHsv.v
            );
            ...
            const primaryHex = rgbToHex(
                finalPrimaryRgbClamped.r,
                finalPrimaryRgbClamped.g,
                finalPrimaryRgbClamped.b
            );
            const secondaryHex = rgbToHex(
                finalSecondaryRgbClamped.r,
                finalSecondaryRgbClamped.g,
                finalSecondaryRgbClamped.b
            );
            ...
            if (!primaryHSL || !secondaryHSL) {
                ...
                console.error(
                    "Failed to convert clustered RGB to HSL. Primary RGB:",
                    finalPrimaryRgbClamped,
                    "Secondary RGB:",
                    finalSecondaryRgbClamped
                );
                ...
                if (initialPoints.length > 0) {
                    ...
                    const rgbFromHsv = hsvToRgb(
                        initialPoints[0].hsv.h,
                        initialPoints[0].hsv.s,
                        initialPoints[0].hsv.v
                    );
                    const fallbackHex = rgbToHex(
                        rgbFromHsv.r,
                        rgbFromHsv.g,
                        rgbFromHsv.b
                    );
                ...
                reject(
                    new Error(
                        "Failed to convert clustered RGB colors to HSL and no fallback."
                    )
                );
                return;
            }
