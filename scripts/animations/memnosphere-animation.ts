...
export function playMemnosphereAnimation(memnosphereData: { itemName: string, rarity: string, imageUrl: string | null, effects?: string[] }): Promise<void> {
    return new Promise(async (resolve) => {
        ...
        if (DEV_MODE) {
            ...
            animationContainer.addEventListener('click', clickHandler, { once: true });        }        // --- Initialize WebGPU Particles Background ---
        ...
        try {
            ...
            const particlesInitialized = await webgpuParticles.init(animationContainer);
        ...
        const burstContainer = createElement('div', ['burst-particle-container'], {
            width: '100%',
            height: '100%',
            position: 'absolute',
            top: '0',
        ...
        for (let i = 0; i < numBurstParticles; i++) {
            let color = utils.randomPick([ColorPalette.variations.primaryBright(0.7), ColorPalette.variations.secondaryBright(0.7)])

            const size = utils.random(4, 50)
            const particle = createElement('div', ['burst-particle'], {
                position: 'absolute',
                left: "50%", 
                top: "50%",
                width: `${size}px`,
            ...
            const distance = utils.random(particleMaxBaseDistance * 0.4, particleMaxBaseDistance); 
            ...
            phaseC.add(particle, {
                ...
                opacity: [
                    ...
                    { to: 1, duration: particleBurstDuration - 150 - 400, easing: 'linear' },
                    { to: 0, duration: 400, easing: 'inQuad' } 
                ],
                scale: [ 
                    { to: utils.random(0.3, 0.7), duration: 0 },
                    { to: utils.random(0.8, 1.5), duration: particleBurstDuration * 0.4, easing: 'outQuad' },
                    { to: utils.random(0.1, 0.4), duration: particleBurstDuration * 0.6, easing: 'inQuad' }
                ],
        ...
        phaseC.add(itemImageElement, { // This animates the main container (glass sphere + overlay)
            opacity: [0, 1],
            scale: [0.3, 1.1, 1],
        ...
        phaseC.add(itemNameText, {
            opacity: [0, 1],
            translateY: ['20px', '0px'], // Slide up
            translateX: '-50%',
            duration: 600,
            ease: 'outQuint'
        }, "-=500"); // Overlap with item settling
