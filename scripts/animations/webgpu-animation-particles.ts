...
declare var WebGPUParticles: {
    new(canvas: HTMLCanvasElement, fpsDisplay: HTMLElement, errorDisplay: HTMLElement): {
        init: () => Promise<void>;
        toggle: () => void;
        isRunning: boolean;
...
export class WebGPUAnimationParticles {
    ...
    async init(parentContainer: HTMLElement): Promise<boolean> {
        ...
        if (!parentContainer) {
            console.error('Parent container is required for WebGPUAnimationParticles initialization.');
            return false;
        }
        ...
        if (typeof WebGPUParticles === 'undefined') {
            console.error("Global 'WebGPUParticles' class from webgpu.js not found. Cannot initialize particle system.");
            if (this.errorDisplayElement) {
                this.errorDisplayElement.textContent = "Error: WebGPUParticles class not found. Check console.";
        ...
        try {
            this.particleSystem = new WebGPUParticles("modules/fabula-ultima-technosphere-machine/build/webgpu-particles.wasm", this.canvas, this.fpsDisplayElement, this.errorDisplayElement);
     ...
     */
    private setupWebGPUElements(container: HTMLElement): { canvas: HTMLCanvasElement, fpsDisplay: HTMLElement, errorDisplay: HTMLElement } {
        // Canvas Element
        ...
        if (!canvas) {
            ...
            console.warn('WebGPUAnimationParticles: Global canvas with id="canvas" exists but is not in the target container. Moving it.');
            if (container.firstChild) {
                container.insertBefore(canvas, container.firstChild);
            } else {
                container.appendChild(canvas);
    ...
    start(): void {
        if (!this.particleSystem) {
            ...
            if (!this.parentContainer) {
                 console.error("WebGPUAnimationParticles: Cannot start, not initialized and no parent container to re-init.");
                 return;
            }
            console.warn("WebGPUAnimationParticles: Attempting to start, but system not initialized. Trying to init again.");
            this.init(this.parentContainer).then(success => {
                if (success && this.particleSystem && !this.particleSystem.isRunning) {
    ...
    destroy(): void {
        ...
        if (this.fpsDisplayElement && this.fpsDisplayElement.parentElement) {
            this.fpsDisplayElement.parentElement.removeChild(this.fpsDisplayElement);
        }
        if (this.errorDisplayElement && this.errorDisplayElement.parentElement) {
            this.errorDisplayElement.parentElement.removeChild(this.errorDisplayElement);
        }
        this.canvas = null;
