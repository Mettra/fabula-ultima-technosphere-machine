...
async function addMnemosphereSourceIcons(html: any, actor: any) {
    ...
    for (const item of generatedItems) {
        const sourceUuids = item.getFlag(ModuleName, "mnemosphere-sources");
        if (!sourceUuids || sourceUuids.length === 0) continue;

        // Find the item row in the HTML
        const itemRow = html.find(`li[data-item-id="${item.id}"]`);
        ...
        const iconsHtml = sourceMnemospheres.map(source => `
            <img src="${source.img}"
        ...
        if (itemNameDiv.length > 0) {
            itemNameDiv.append(`<span class="mnemosphere-sources">${iconsHtml}</span>`);
        }
    }
        ...
        const cardHtml = `
            ...
            ">
                <div style="display: flex; align-items: center; margin-bottom: 4px;">
                    <strong>${mnemosphere.name}</strong>
                </div>
            </div>
        `;
